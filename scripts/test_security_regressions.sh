...
for qml_file in "$MAIN_QML" "$PROVIDERS_QML" "$POPUP_QML" "$DIAGNOSTICS_QML"; do
...
require_in_surface popup "plainText: providerMoveUp.Accessible.name"
require_in_surface popup "plainText: providerMoveDown.Accessible.name"
...
require_in_surface panel "text: SafeText.plainTextAsRichText(modelData.displayName)"
...
require_in_surface providers "var plan = ProviderDescriptor.planFieldWrite(field, value, commandPath)"
require_in_surface providers "var plan = ProviderDescriptor.planSecretPrompt(field, commandPath)"
require_in_surface providers "var plan = ProviderDescriptor.planAction(action, commandPath)"
