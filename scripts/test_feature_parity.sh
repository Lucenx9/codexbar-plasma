...
require_in_file "$PROVIDERS_QML" "ProviderConfigProtocol.descriptorUnsupportedMessage(stdoutText, stderrText)"
require_in_file "$PROVIDERS_QML" "ProviderConfigProtocol.commandError(payload)"
reject_in_file "$PROVIDERS_QML" "stdoutText + \"\\n\" + stderrText"
require_in_surface providers "function normalizeProviderList(payload, fallbackTitleResolver)"
require_in_file "$PROVIDERS_QML" "ProviderConfigProtocol.normalizeProviderList(payload"
require_in_surface providers "function normalize(raw, fallbackFieldTitle)"
...
require_in_surface providers "function planFieldWrite(field, value, commandPath)"
require_in_surface providers "function planSecretPrompt(field, commandPath)"
require_in_surface providers "selectedOptionIndex: optionIndex(options, normalizedValueText)"
require_in_surface providers "i === 0 && token === \"codexbar\""
require_in_file "$PROVIDERS_QML" "function handleDiagnoseResult(descriptor, stdoutText, stderrText)"
require_in_surface providers "function normalizeProviderDiagnostic(payload)"
require_in_file "$PROVIDERS_QML" "ProviderConfigProtocol.normalizeProviderDiagnostic(payload)"
...
require_in_surface providers "descriptor: ProviderDescriptor.normalize(item.descriptor"
...
require_in_surface applet "function usageDashboard(usage, item)"
...
require_in_surface applet "var providerUsageDashboard = providerDetails.length > 0 ? null : usageDashboard(usage, item)"
...
require_in_surface applet "LegacyUsageDashboard.normalize(usage, item)"
...
require_in_file "$NOTIFICATIONS_QML" "Enable Plasma notifications"
require_in_file "$NOTIFICATIONS_QML" "Notify status incidents"
require_in_file "$NOTIFICATIONS_QML" "Notify quota warnings"
require_in_file "$NOTIFICATIONS_QML" "Notify predicted quota exhaustion"
require_in_file "$NOTIFICATIONS_QML" "Notify limit resets"
...
require_in_file "$PANEL_QML" "Panel text:"
require_in_file "$PANEL_QML" "Show provider name in panel"
require_in_file "$PANEL_QML" "Show usage text in panel"
...
require_in_file "$PANEL_QML" "Show provider meters in panel"
...
require_in_file "$DIAGNOSTICS_QML" "Advanced provider override"
require_in_file "$DIAGNOSTICS_QML" "Provider id (blank = all enabled)"
require_in_file "$DIAGNOSTICS_QML" "Provider default (blank)"

require_in_file "$DIAGNOSTICS_QML" "function runDiagnostic()"
require_in_file "$DIAGNOSTICS_QML" "diagnose --provider"
require_in_file "$DIAGNOSTICS_QML" "--format json --redact"
require_in_file "$DIAGNOSTICS_QML" "id: diagnosticOutputArea"
...
require_in_file "$README_MD" "Panel text modes"
...
require_in_file "$README_MD" "Usage refresh choices"
