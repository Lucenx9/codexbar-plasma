...
function cliVersionAtLeast(value, requiredMajor, requiredMinor, requiredPatch) {
    ...
    var match = text.match(/(?:^|\s)v?(\d{1,3})\.(\d{1,3})\.(\d{1,3})((?:[-+~][0-9A-Za-z.+~_-]+)?)(?:\s|$)/)
    if (!match) {
        return false
    }
    ...
    // Build metadata and numeric distro revisions keep the release capability.
    // Other hyphen suffixes and tilde revisions precede the matching release.
    var suffix = match[4]
