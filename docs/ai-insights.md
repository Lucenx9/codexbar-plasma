...
- Spend and token comparisons require the cost history already loaded for the
  **Usage & Spend** range: all fourteen complete days before today, each with a
  ...
  never summed across providers. Spend keeps the qualifier the widget shows
  beside the amount: estimated or approximate sources, including an estimated
  share, set `estimated`, and partial amounts (unpriced or unmetered requests)
  or days with incomplete requests set `incomplete`. Opening the popup or
