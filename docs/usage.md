...
- **Quota weeks** in the expanded provider details split the local history by
  ...
  earlier ones, each with its exact start and end. The boundaries come from the
  ...
  marked `≈`. Unknown days make a total read "at least"; weeks reaching back
  before the scanned history range are left out rather than shown incomplete.
